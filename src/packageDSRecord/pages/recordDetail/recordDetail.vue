        ...
        <view class="content">
          <view class="record-order">
            <view class="record-view-info">
              ...
              <!-- 支付方式: 订单状态未已关闭、待付款时不显示支付方式，其他状态显示支付方式 -->
              <view class="order-info-line" v-if="(detail.orderStatus!=4 && detail.orderStatus!=0) || (detail.orderStatus==0 && detail.payStatus=='05')">
                <view class="order-label">支付方式</view>
                <view class="order-words">{{detail.payName}}</view>
              </view>
            </view>
